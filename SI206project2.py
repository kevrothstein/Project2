...
def grab_headlines():
    url = 'file:///Users/kevinrothstein/Desktop/Project2-master/opinion.html'
    lst = []
    r = urllib.request.urlopen(url).read()
    B_soup = BeautifulSoup( r , 'html.parser')
    soup_find = B_soup.find( class_= "view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266")
    soup_find_all = soup_find.findall('li')
    for words in soup_find_all:
        if words.string != '\n':
            lst.append(words.a.text)
    return lst        
...
def get_umsi_data():
    umsi_titles = {}
    the_first_url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All'
    diff_pages = 0
    r = requests.get(the_first_url, headers = {'User-Agent': 'SI_CLASS'})
    ...
    while (diff_pages < 13):
        ...
        count = 0
        h2 = B_soup('h2')
        div_tag = B_soup.find_all('div', {"class": "field-item even" })

        for kevin in div_tag:
            if kevin.string != None:
                first_titles.append(kevin.string)

        for kevin2 in range(len(first_titles)):
            if kevin2 % 2 != 0: 
                second_titles.append(first_titles[kevin2])

        for kevin3 in h2[3:len(h2) - 2]:
            if '.' not in kevin3.string:
                the_names.append(kevin3.string)

        for kevin4 in the_names:
            umsi_titles[kevin4] = second_titles[count]
            count += 1
        diff_pages += 1
        second_url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=' + str(diff_pages)
        r = requests.get(second_url, headers = {'User-Agent': 'SI_CLASS'})
        B_soup = BeautifulSoup(r.content, 'html.parser')
    return umsi_titles
...
def num_students(data):
    count = 0
    for phd in data.keys():
        if data.phd == 'PhD student':
            count += 1
    return count 
    #Your code here
